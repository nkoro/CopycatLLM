...
# ==========================================
# STAGE 1: HARVEST & STAGE 2: TELEMETRY
# ==========================================
...
def stage_two_telemetry():
    ...
    if telemetry_path.exists():
        print("✓ Telemetry dataset already exists. Skipping inference to save time.")
        return
...
def run_comprehensive_ablations(df):
    ...
    def evaluate_pipeline(scores, df_eval, use_hdbscan=True, static_thresh=0.50, epsilon=0.02):
        if use_hdbscan:
            ...
            if len(valid_idx) == 0:
                logging.warning("HDBSCAN labeled all points as noise. Falling back to 95th percentile.")
    ...
    # ACT 2: PURE UNSUPERVISED CLUSTERING
    ...
    # ACT 3: PROPOSED HYBRID OPTIMIZER (Micro-Sweep from Anchor to 1.0)
    best_mcc = -1.0
    best_tau = hdbscan_tau
    ...
    print("-" * 95)
    print("--- ACT 2: PURE UNSUPERVISED CLUSTERING ---")
    ...
    print("-" * 95)
    print("--- ACT 3: PROPOSED HYBRID OPTIMIZER ---")
    ...
    print(f"\n✓ Comprehensive JSON with updated Stratified Metrics saved to: {ABLATION_DIR}/comprehensive_ablation_metrics.json")
...
def generate_high_res_3d_plots(df):
    ...
    plt.rcParams.update({
        'font.family': 'serif', 'font.size': 11, 'axes.linewidth': 1.5,
        'axes.edgecolor': 'black', 'figure.dpi': 300, 'savefig.dpi': 300, 'grid.alpha': 0.5
    })
    ...
    # PLOT 1 (JUDGE SPECIFIC FILENAME)
    ...
    # PLOT 2 (JUDGE SPECIFIC FILENAME)
    ...
    print(f"✓ Dual 3D High-Res PNGs saved to {ABLATION_DIR}/")
