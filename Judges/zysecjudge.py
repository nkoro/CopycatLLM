...
# ---> ISOLATED SUB-FOLDER FOR ZYSEC <---
ABLATION_DIR = OUTPUT_DIR / "zysec_eval"
ABLATION_DIR.mkdir(exist_ok=True, parents=True)

JUDGE_ID = "/home/enkoro1/models/zysec-7b-v1.Q4_K_M.gguf" 
...
class NeuralSemanticGateway:
    def __init__(self):
        print(f"\nLoading GGUF Semantic Judge ({JUDGE_ID}) via llama.cpp...")
        self.model = Llama(
            model_path=JUDGE_ID,
            n_gpu_layers=-1, 
            n_ctx=2048,
    ...
    def analyze_prompt(self, prompt):
        ...
        unified_prompt = f"{self.system_prompt}\n\nUser Input: {safe_prompt}\n\nOutput strictly as JSON:"
        ...
        try:
            clean_resp = text_out.strip().strip("```json").strip("```").strip()
            # Failsafe parsing for generated artifacts
...
def stage_two_telemetry(gateway):
    print("\n=== STAGE 2: STARTING BLIND TELEMETRY COLLECTION (ZYSEC) ===")
    telemetry_path = ABLATION_DIR / "zysec_mixed_telemetry.json"
    
    if telemetry_path.exists():
        print("✓ Zysec Telemetry dataset already exists. Skipping inference to save time.")
        return
    ...
    print("\n✓ Zysec Telemetry collection complete and saved.")
...
def run_eti_evaluation():
    ...
    with open(ABLATION_DIR / "zysec_mixed_telemetry.json", "r") as f:
...
def run_comprehensive_ablations(df):
    ...
    with open(ABLATION_DIR / "zysec_ablation_metrics.json", "w") as f:
        json.dump(ablation_results, f, indent=4)
    print(f"\n✓ Comprehensive JSON saved to: {ABLATION_DIR}/zysec_ablation_metrics.json")
...
def generate_high_res_3d_plots(df):
    print("\n[!] Generating Dual 3D Latent Space Projections (Zysec)...")
    ...
    ax1.set_title("3D Latent Space: Binary Gateway Decision (Zysec Judge)", pad=20, fontweight='bold')
    format_axes(ax1)
    plt.legend(loc='upper right', bbox_to_anchor=(1.1, 1), framealpha=1.0, edgecolor='black')
    plt.tight_layout()
    plt.savefig(ABLATION_DIR / "zysec_3d_latent_space_binary.png", format='png', bbox_inches='tight')
    plt.close()
    ...
    ax2.set_title("3D Latent Space: Multiclass Signatures (Zysec Judge)", pad=20, fontweight='bold')
    format_axes(ax2)
    plt.legend(loc='upper right', bbox_to_anchor=(1.1, 1), framealpha=1.0, edgecolor='black')
    plt.tight_layout()
    plt.savefig(ABLATION_DIR / "zysec_3d_latent_space_multiclass.png", format='png', bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    print("\nExperiment Phase A (Zysec Judge) completed successfully.")
