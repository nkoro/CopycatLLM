...
# ---> ISOLATED SUB-FOLDER FOR GEMMA <---
ABLATION_DIR = OUTPUT_DIR / "gemma_eval"
ABLATION_DIR.mkdir(exist_ok=True, parents=True)

JUDGE_ID = "/home/enkoro1/gemma-4-12B-local" 
DEVICE_JUDGE = "cuda:0"
...
class NeuralSemanticGateway:
    def __init__(self):
        print(f"\nLoading LLM Semantic Judge ({JUDGE_ID}) onto {DEVICE_JUDGE}...")
    ...
    def analyze_prompt(self, prompt):
        ...
        unified_prompt = f"System Instruction: {self.system_prompt}\n\nUser Input: {safe_prompt}"
        ...
        try:
            ...
            return {
                "has_cot": 1.0 if "step" in p_lower or "reason" in p_lower else 0.0,
...
def stage_two_telemetry(gateway):
    print("\n=== STAGE 2: STARTING BLIND TELEMETRY COLLECTION (GEMMA) ===")
    telemetry_path = ABLATION_DIR / "gemma_mixed_telemetry.json"
    
    if telemetry_path.exists():
        print("✓ Gemma Telemetry dataset already exists. Skipping inference to save time.")
        return
    ...
    print("\n✓ Gemma Telemetry collection complete and saved.")
...
def run_eti_evaluation():
    ...
    with open(ABLATION_DIR / "gemma_mixed_telemetry.json", "r") as f:
...
def run_comprehensive_ablations(df):
    ...
    with open(ABLATION_DIR / "gemma_ablation_metrics.json", "w") as f:
        json.dump(ablation_results, f, indent=4)
    print(f"\n✓ Comprehensive JSON saved to: {ABLATION_DIR}/gemma_ablation_metrics.json")
...
def generate_high_res_3d_plots(df):
    ...
    ax1.set_title("3D Latent Space: Binary ETI Gateway Decision (Gemma Judge)", pad=20, fontweight='bold')
    format_axes(ax1)
    plt.legend(loc='upper right', bbox_to_anchor=(1.1, 1), framealpha=1.0, edgecolor='black')
    plt.tight_layout()
    plt.savefig(ABLATION_DIR / "gemma_3d_latent_space_binary.png", format='png', bbox_inches='tight')
    plt.close()
    ...
    ax2.set_title("3D Latent Space: Multiclass Traffic Signatures (Gemma Judge)", pad=20, fontweight='bold')
    format_axes(ax2)
    plt.legend(loc='upper right', bbox_to_anchor=(1.1, 1), framealpha=1.0, edgecolor='black')
    plt.tight_layout()
    plt.savefig(ABLATION_DIR / "gemma_3d_latent_space_multiclass.png", format='png', bbox_inches='tight')
    plt.close()
